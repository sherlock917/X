...
class Smarty_Internal_Compile_Include extends Smarty_Internal_CompileBase {
	...
	public function compile($args, $compiler, $parameter) {
		...
		/*
		 * if the {include} tag provides individual parameter for caching it will not be included into the common cache file and treated like a nocache section
		 */
		...
		if ($merge_compiled_includes) {
			...
			if (! isset ( $compiler->smarty->merged_templates_func [$tpl_name] [$uid] )) {
				...
				$compiler->smarty->merged_templates_func [$tpl_name] [$uid] ['func'] = $tpl->properties ['unifunc'] = 'content_' . str_replace ( array (
						'.',
						',' 
				), '_', uniqid ( '', true ) );
